...
async function autoCapture(page, m365Url, discovered, demoDir, generatedScript = null, headless = false, captureOpts = {}, runOpts = {}) {
  ...
  async function capturePlatformInNewTab(conn, ssPath) {
    ...
    try {
      ...
      if (platform === 'sharepoint') {
        ...
        const emailOpened = await tab.evaluate(() => {
          const SUBJECT_KEYWORDS = ['onboarding', 'engagement', 'client', 'plumbing', 'jovic'];
          ...
          for (const row of rows) {
            ...
            if (SUBJECT_KEYWORDS.some(kw => t.includes(kw))) {
              row.click();
              return true;
            }
          }
  ...
  try {
    ...
    if (!chatLoaded) {
      ...
      // Strategy 1: AI-generated conversation script (guided by prompts if provided)
